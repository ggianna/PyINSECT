...
class BinaryOperator(NaryOperator):
    ...
    def apply(self, *args):
        ...
        if l != 2:
            raise ValueError(
                "Binary operators accept exactly two arguments!\n" + str(l) + " given."
            )
    ...
    def __init__(self, op, nthreads=0):
        ...
        try:
            if not op._distributable:
                warnings.warn(
                    "Given operator is not defined as distributable.\nResult may be false.",
                    UserWarning,
                )
            ...
            if self._nthreads > 0:
                for (a, b) in l:
                    thr = threading.Thread(
                        ...
                        args=(self._Op.apply, a, b, dc, fargs, str(i), lock),
                    )
                    threads.append(thr)
                    i += 1
